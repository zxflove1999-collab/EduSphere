...
> 说明：虽然宿舍问卷属于“校园生活”模块，但预约系统前端也需要读取该数据用于展示学生画像，因此在此同步记录。
...
#### A.1.1 基本信息
  ...
  * **请求方式：** `POST`
  * **接口描述：** 写入 / 更新 `t_dorm_survey`，学生提交最新宿舍偏好。
...
| `smoking_status` | Integer | 否 | 抽烟情况 |
| `roommate_smoke_accept` | Integer | 否 | 对室友抽烟的接受度 |
| `gaming_freq` | Integer | 否 | 游戏频率 |
| `headphone_usage` | Integer | 否 | 耳机使用习惯 |
| `chatting_pref` | Integer | 否 | 聊天偏好 |
| `guest_acceptance` | Integer | 否 | 带客接受度 |
...
| `noise_tolerance_level` | Integer | 否 | 噪音容忍等级 |
| `dorm_food_freq` | Integer | 否 | 宿舍吃东西频率 |
| `fragrance_usage` | Integer | 否 | 香氛使用 |
